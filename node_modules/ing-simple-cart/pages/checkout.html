<!DOCTYPE html>
<html lang="en">
  <head>
    <title>ING Shopping Cart Challenge ES^6</title>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no" />
    <link type="image/x-icon" rel="shortcut icon" href="../assets/image/favicon.ico" />
    <link rel="stylesheet" href="../assets/font-awesome/css/font-awesome.min.css" />
    <link rel="stylesheet" href="../assets/css/bootstrap.min.css" />
    <link rel="stylesheet" href="../assets/css/style.css" />

    <script
      src="https://unpkg.com/ing-simple-cart@1.0.20/dist/ing-simple-cart.min.js"
      type="text/javascript"
    ></script>
  </head>

  <header>
    <div
      class="d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 ing-bg border-bottom shadow-sm"
    >
      <img src="../assets//image/ing-logo-full.svg" width="150" />
      <h2 class="my-0 mr-md-auto font-weight-normal ing-title-header">Shopping-Cart-Challenge</h2>

      <a class="btn btn-outline-light mr-2 cart-length" href="cart.html"></a>
      <a class="btn btn-outline-dark" href="https://github.com/mani242530/ing-simple-cart"
        >Github Repo</a
      >
    </div>
  </header>

  <div class="container padding-bottom-3x mb-1 body-content">
    <div class="row flex-nowrap">
      <div class="col-12 col-sm-6 col-md-4 col-lg-12">
        <h4>Billing Address</h4>
        <hr class="padding-bottom-1x" />

        <form novalidate="" class="userForm">
          <div class="user-details"></div>
          <div class="address"></div>
        </form>
        <div class="shopping-cart-footer">
          <div class="column">
            <a class="btn btn-outline-secondary" href="cart.html">
              <i class="icon-arrow-left"></i>&nbsp;Back
            </a>
          </div>
          <div class="column">
            <a class="btn btn-success" href="payment.html">Continue</a>
          </div>
        </div>
      </div>
    </div>
  </div>

  <script>
    // renderUser
    renderUser = user => {
      const $user = document.querySelector('.user-details');
      const $address = document.querySelector('.address');

      $user.innerHTML = ` <div class="row">
              <div class="col-sm-6">
                <div class="form-group">
                  <lion-input label="First Name" value="${user.firstName}"></lion-input>
                </div>
              </div>
              <div class="col-sm-6">
                <div class="form-group">
                  <lion-input label="Last Name" value="${user.lastName}"></lion-input>
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col-sm-6">
                <div class="form-group">
                  <lion-input label="E-mail Address" value="${user.personalEmailAddress}"></lion-input>
                </div>
              </div>
              <div class="col-sm-6">
                <div class="form-group">
                  <lion-input label="Phone Number" value="${user.phoneNumber}"></lion-input>
                </div>
              </div>
            </div>`;

      $address.innerHTML = user.addresses
        .map(
          address =>
            `<div class="row">
              <div class="col-sm-6">
                <div class="form-group">
                  <lion-input label="Street 1" value="${address.street1}"></lion-input>
                </div>
              </div>
              <div class="col-sm-6">
                <div class="form-group">
                  <lion-input label="Street 2" value="${address.street2}"></lion-input>
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col-sm-6">
                <div class="form-group">
                  <lion-input label="Postal Code" value="${address.postalCode}"></lion-input>
                </div>
              </div>
              <div class="col-sm-6">
                <div class="form-group">
                  <lion-input label="City" value="${address.city}"></lion-input>
                </div>
              </div>
            </div>`,
        )
        .join('');
    };

    renderUser(shoppingCart.user());

    // renderCartLength
    renderCartLength = item => {
      const $cartLength = document.querySelector('.cart-length');
      $cartLength.innerHTML = `<span>Cart(${item})</span>`;
    };

    renderCartLength(shoppingCart.cartCount());
  </script>

  <footer class="footer">
    <div class="container">
      <div class="copyright">© 2021 ING SIMPLE CART, all rights reserved.</div>
    </div>
  </footer>
</html>
